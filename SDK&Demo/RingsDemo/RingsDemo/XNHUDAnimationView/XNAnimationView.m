    ...
    if (_loadingLayer) {
        if ([_pointLayers containsObject:_loadingLayer]) {
            ...
            [_pointLayers enumerateObjectsUsingBlock:^(CALayer<XNHUDLayerProtocol>* obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if (obj == wkSelf.loadingLayer) {
                    [obj stop];
                    [wkSelf.pointLayers replaceObjectAtIndex:idx withObject:loadingLayer];
                }
            }];
        }
    }
...
- (void)setFrame:(CGRect)frame {
    ...
    [_pointLayers enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        layer.targetLayer = wkSelf.layer;
        layer.xn_strokeColor =  wkSelf.tintColor.CGColor;
        layer.xn_lineWidth = wkSelf.lineWidth;
        layer.timingFunction = wkSelf.timingFunction;
