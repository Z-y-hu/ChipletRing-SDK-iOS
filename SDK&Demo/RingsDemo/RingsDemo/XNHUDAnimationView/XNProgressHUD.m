...
- (void)didDisplayOnMainQueue {
    ...
    [UIView animateWithDuration:_duration animations:^{
        ...
        if(showing) {
            [wkSelf update];
            wkSelf.animationView.alpha = 1.f;
        }else{
            wkSelf.maskView.alpha = 1.f;
            wkSelf.shadeContentView.alpha = 1.f;
        }
    } completion:^(BOOL finished) {
        wkSelf.disposableDelayResponse = 0.f;
    }];
...
- (void)didDismissOnMainQueue {
    ...
    [UIView animateWithDuration:self.duration animations:^{
        if(wkSelf.isMaskEnable && wkSelf.maskView)
            wkSelf.maskView.alpha = 0.f;
        wkSelf.shadeContentView.alpha = 0.f;
    } completion:^(BOOL finished) {
        if ([self isWindowAndIsNotKeyWindow:(targetView)]) {
            targetView.hidden = YES;
        }
        [wkSelf removeFromSuperview:wkSelf.shadeContentView];
        [wkSelf removeFromSuperview:wkSelf.maskView];
        if(wkSelf.hudDismissBlock)
            [wkSelf removeMaskTapGestureEvent];
        wkSelf.disposableDelayDismiss = 0.f;
        wkSelf.maskType = XNProgressHUDMaskTypeNone;
    }];
}
